...
var getDetailInfo = function(d){
    ...
    for (var i = 0; i < d.tiles.length; i++) {
        if (d.tiles[i].type == "pic") {
            ...
            _h+='<img class="lazy"';
            if(weTao.isDesktop){
                _h+='dataimg="'+ getBetterImg(tiles[i].path, 370, parseInt(tiles[i].picWidth),true) +'"';
            }else{
                _h+='dataimg="'+ getBetterImg(tiles[i].path, window.innerWidth-30, parseInt(tiles[i].picWidth)) +'"';
                _h+='style="'+ detailImageSizeStyle(tiles[i].picWidth, tiles[i].picHeight) +'"';
            }
...
var getTemplate = function(param,success,fail){
    ...
    namespace('taobao.utils.server.simplemtop').getApi("mtop.sns.component.template",
        '2.0',
...
var getDetailByComponent = function (){
...
$(function(){
    ...
    namespace('taobao.utils.server.simplemtop').getApi("mtop.sns.pubAccount.info",
        ...
        function (result) {
            //success
            var d = result.data;
            $('#detailPage div.account').html(getAccountInfo(d));
        },
        function(result){
            //fail
    ...
    namespace('taobao.utils.server.simplemtop').getApi("mtop.sns.feed.detail",
        ...
        function (result) {
            //success
            console.log(result);
            $('#detailPage div.main').html(getDetailInfo(result.data));
